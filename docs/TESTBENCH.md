...
Preview artifacts are generated only when you dispatch a preview from the CLI,
TUI, or GitHub Actions UI. Ordinary `dev` pushes are just checkpoint commits;
they do not publish review artifacts.
...
Boardwright defaults review to `variants.preview_default`, not the source
project's `variants.dev_default` design-stage label. Use the variant selector
or `--variant` when manually dispatching a different CI output variant. If the
source project itself has moved from `DRAFT` to `PRELIMINARY` or `CHECKED`,
change `variants.dev_default` deliberately in Project Info and commit it as a
normal source-state change.
...
The PCB keeps movable KiBot table placeholder groups so table placement can be
edited directly in KiCad. Testpoint and drill tables are included through the
`pcb_print` output-level `include_table` option, not the global `include_table`
preflight. The assembly component-count table is prepared by Boardwright before
KiBot runs, because KiBot's `report` output can recursively call `pcb_print`
when used as an include-table source. CI therefore runs
`prepare_pcb_tables.py` first, then runs KiBot's `table_sources` target for the
remaining non-recursive CSVs, then runs the full output target. Impedance is
intentionally not included by default until Boardwright has a
controlled-impedance data model.
...
The promote action dispatches `main-outputs.yaml` from the reviewed `dev`
source SHA, verifies that SHA in CI, generates the accepted package, and then
pushes the accepted source plus README/render snapshot to `main` when commit
outputs is enabled.
...
Accepted-main evidence must be fresh for the latest pushed `origin/dev` source
SHA before release testing. If `dev` moves after acceptance, accept again before
creating a release.
