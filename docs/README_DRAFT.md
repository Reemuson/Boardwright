...
The main TUI groups these actions as Work, Preview, Release, and Setup. The
center panel is a compact workflow map; the right panel shows the current next
move, preview/accepted evidence, and release readiness.
...
- Accept to Main builds from the exact reviewed `dev` source SHA and pushes
  that source plus accepted README/render snapshot assets to `main`.
